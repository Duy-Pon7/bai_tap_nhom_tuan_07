  ...
  return (
    <div className="w-full overflow-x-hidden p-4 text-gray-800 dark:text-gray-100">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="ml-auto text-sm text-gray-500 dark:text-gray-400 font-medium">ADMIN PANEL</span>
      </div>

      {/* Config card */}
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide font-semibold">Đăng nhập với</div>
            <div className="text-sm font-medium mt-1 text-gray-800 dark:text-white/90">
              {currentUser
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            >🔌 Connect WS</button>
            <button
              className="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-semibold transition"
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 mt-2 font-mono">
          Room: <b className="text-blue-600 dark:text-blue-400">{activeRoom ? activeRoom.slice(0,16) + "…" : "(chưa chọn)"}</b>
          &nbsp;|&nbsp; AdminId: <b className="text-blue-600 dark:text-blue-400">{adminIdRef.current || "(chưa rõ)"}</b>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-12 lg:col-span-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">
              Danh sách room HUMAN
            </h3>
            <div className="max-h-[520px] overflow-y-auto border border-gray-100 rounded-lg dark:border-gray-800 dark:bg-gray-900/20">
              {rooms.length === 0
                ? <div className="p-3 text-sm text-gray-400 dark:text-gray-500">(Chưa load)</div>
                : rooms.map(r => {
                    const id  = r.id || r._id;
                    ...
                    return (
                      <div
                        key={id}
                        onClick={() => openRoom(id)}
                        className={`p-3 border-b border-gray-100 dark:border-gray-800 last:border-b-0 cursor-pointer transition
                          ${isActive
                            ? "bg-blue-50 border-l-4 border-blue-600 dark:bg-blue-500/15"
                            : hasUnread
                            ? "bg-red-50 border-l-4 border-red-500 dark:bg-red-500/10"
                            : "hover:bg-gray-50 dark:hover:bg-white/5"
                          }`}
                      >
                        <div className="flex items-center gap-1 flex-wrap">
                          <span className={`font-semibold text-sm ${hasUnread && !isActive ? "text-red-600 dark:text-red-400" : "text-gray-800 dark:text-gray-100"}`}>
                            {r.userId || "(no userId)"}
                          </span>
                          <span className={`px-2 py-0.5 rounded text-xs font-bold
                            ${st === "OPEN" ? "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300"}`}>
                            {st}
                          </span>
                          {/* Unread badge */}
                          {hasUnread && !isActive && (
                        ...
                        <div className="text-xs text-gray-400 dark:text-gray-500 font-mono mt-1 break-all">id: {id}</div>
        ...
        <div className="col-span-12 lg:col-span-8">
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col h-[580px] dark:bg-white/[0.03] dark:border-gray-800">
            <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Chat</h3>
              {activeRoom && (
                <span className="text-xs text-gray-400 dark:text-gray-500 font-mono">{activeRoom.slice(0,16)}…</span>
              )}
            </div>

            {/* Messages */}
            <div ref={chatBoxRef} className="flex-1 overflow-y-auto p-4 bg-gray-50 dark:bg-gray-900/40 min-h-0">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-sm gap-2">
                ...
                <div key={idx} className={`flex flex-col mb-4 max-w-[72%] ${m.isMe ? "ml-auto items-end" : "mr-auto items-start"}`}>
                  <div className="text-xs text-gray-400 dark:text-gray-500 font-mono mb-1 px-1">
            ...
            <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800 flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 border border-gray-200 bg-white text-gray-800 placeholder:text-gray-400 rounded-lg text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder:text-gray-500 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 transition"
              />
              <button
                onClick={sendMsg}
      ...
      {/* Log */}
      <div className="bg-white border border-gray-200 rounded-xl p-4 mt-4 shadow-sm dark:bg-white/[0.03] dark:border-gray-800">
        <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Log</h3>
        <pre className="h-[120px] overflow-auto bg-gray-900 text-gray-300 dark:bg-black/40 dark:text-gray-200 p-3 rounded-lg text-xs whitespace-pre-wrap break-words font-mono">
          {logs || "(trống)"}
        </pre>
      </div>
